...
3. **Combining with Pauses**: Audio segments are combined sequentially with configurable pauses (default 800ms) between them for natural breaks
4. **Normalization**: Final audio is normalized for consistent volume across all segments
5. **Single Output**: Exported as a single high-quality MP3 file (192k bitrate)
